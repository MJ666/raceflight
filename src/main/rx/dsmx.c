...
#include "dsmx.h"
#include "drivers/rx_cyrf6936.h"
#include "drivers/rx_cyrf6936_regs.h"
// #include "comm.h"
// #include "aq_timer.h"
// #include "util.h"
// #include "config.h"
...
static dsmStruct_t dsmData;
static rxConfig_t *rxConfigPtr;
static rxRuntimeConfig_t *rxRuntimeConfigPtr;
static uint32_t dsmChannelData[12];
...
void dsmBindReceive(int state) {
    ...
    switch (state) {
        ...
        case 2:
            ...
            if (dsmData.packetLen < 16 || packet[0] != packet[4] || packet[1] != packet[5] || packet[2] != packet[6] || packet[3] != packet[7])
                goto bindRcvEnd;
    ...
    for (i = 0; i < 7; i++) {
        ...
        if (chan[0] != 0xff && chan[1] != 0xff) {
            if (dsmData.dsmProtocol & 0x10) {
                ...
                // 10 bit mode
                addr = (chan[0]>>2) & 0x0f;
                val = (((chan[0] & 0x03)<<8) | chan[1]);
            }
            dsmChannelData[addr] = val;
        }
    }
//    r->errorCount = dsmData.rssi & 0x1f;   // actually RSSI (5 bits)
}

uint8_t bindDataSum(void) {
    int i;
    uint8_t sum = 0;

    for (i = 0; i < 6; i++)
        sum += rxConfigPtr->bind_data[i];
    return sum;

}

uint8_t dsmFrameStatus(void) {
    // check for new binding
    if (dsmData.newBind) {
        // store it to flash
        rxConfigPtr->bind_data[0] = dsmData.mfgId[0];
        rxConfigPtr->bind_data[1] = dsmData.mfgId[1];
        rxConfigPtr->bind_data[2] = dsmData.mfgId[2];
        rxConfigPtr->bind_data[3] = dsmData.mfgId[3];
        rxConfigPtr->bind_data[4] = dsmData.dsmProtocol;
        rxConfigPtr->bind_data[5] = dsmData.dsmNumChannels;
        rxConfigPtr->bind_data[6] = bindDataSum();
        saveConfigAndNotify();
        dsmData.newBind = 0;
    }
    // no TX signal, go into bind mode after 15s
    else
        if (cyrfData.initialized && !dsmData.bound && dsmData.state != DSM_RECEIVER_BIND && micros() > 15000000)
            dsmStartBind();
...
static uint16_t dsmReadRawRC(rxRuntimeConfig_t *rxRuntimeConfig, uint8_t chan)
{
    uint16_t data;
    ...
    if (chan >= rxRuntimeConfig->channelCount)
        return 0;
    if (dsmData.dsmProtocol & 0x10)
        data = 988 + (dsmChannelData[chan] >> 1);   // 11 bit mode
    else
        data = 988 + dsmChannelData[chan];          // 10 bit mode
    return data;
}

bool dsmShouldBind(void) {

#ifdef HARDWARE_BIND_PLUG
    // Check status of bind plug and activate bind mode if active and not an softreset
    gpio_config_t cfg = {
        BINDPLUG_PIN,
        Mode_IPU,
        Speed_2MHz
    ...
    if (!digitalIn(BINDPLUG_PORT, BINDPLUG_PIN) && !isMPUSoftReset()) {
        return true;
    }
#else
    // Check spektrum_sat_bind configuration and activate bind mode if not an softreset
    if (rxConfigPtr->spektrum_sat_bind && !isMPUSoftReset())
        rxConfigPtr->spektrum_sat_bind = 0;
        return true;
#endif
    // Check for valid bind data in configuration
    if (rxConfigPtr->bind_data[6] == bindDataSum())
        return false;
    else
        return true;
}
...
bool dsmInit(rxConfig_t *rxConfig, rxRuntimeConfig_t *rxRuntimeConfig, rcReadRawDataPtr *callback) {
    ...
    if (ret) {
        ...
        if (dsmShouldBind())
            ...
            dsmData.mfgId[0] = rxConfigPtr->bind_data[0];
            dsmData.mfgId[1] = rxConfigPtr->bind_data[1];
            dsmData.mfgId[2] = rxConfigPtr->bind_data[2];
            dsmData.mfgId[3] = rxConfigPtr->bind_data[3];
            dsmData.dsmProtocol = rxConfigPtr->bind_data[4];
            dsmData.dsmNumChannels = rxConfigPtr->bind_data[5];
            dsmData.isDsm2 = IS_DSM2(dsmData.dsmProtocol);
            rxRuntimeConfig->channelCount = dsmData.dsmNumChannels;
            dsmStartTransfer();
        }
    }
    if (callback)
        *callback = dsmReadRawRC;
    return ret;
}
#endif
